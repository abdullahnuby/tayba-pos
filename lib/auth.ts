...
// The first admin account is created exclusively through the setup wizard
// (POST /api/auth/setup, gated on db.user.count() === 0). There is
// deliberately no automatic "bootstrap admin with a known password"
// fallback here — that used to run on every login attempt and would create
// an admin/admin123 account for whoever submitted the login form first,
// which could be an attacker instead of the store owner.
